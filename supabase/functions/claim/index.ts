...
const SIGNED_SEC = 300; // app.js 의 SIGNED_SEC 과 맞추기
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const signed = await admin.storage
      .from(BUCKET)
      .createSignedUrl(storage_path, SIGNED_SEC);
    if (signed.error) throw signed.error;

    // download 파라미터를 직접 붙인다.
    // { download: filename } 옵션은 파일명을 이중 인코딩한다.
    const url = `${signed.data.signedUrl}&download=${
      encodeURIComponent(filename)
    }`;

    return json({ url, filename, expires_in: SIGNED_SEC });
  } catch (e) {
    console.error(e);
